...
void FilmService::modifica(string titlu, string gen, const Film& film)
{
	...
	for (const auto& f : filme2) // filmul care urmeaza modificat
	{
		if (f.get_titlu() == titlu && f.get_gen() == gen)
		{
			film2 = f;
...
*/
const vector<Film>& FilmService::getAll()
{
	return rep.getAll();
}
...
*/
bool comparareTitlu(const Film film1, const Film film2)
{
...
*/
bool comparareActorPrincipal(const Film film1, const Film film2)
{
...
*/
bool comparareFilm(const Film& film1, const Film& film2)
{
...
void FilmService::exportHTML(const string& fisier, const vector<Film>& filme)
{
